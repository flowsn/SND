...
export const chapters: Chapter[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      ['Pitch Add In wird am Schrittanfang abgetastet; Änderungen während des Schritts werden nicht übernommen. Bei voll aufgedrehtem Abschwächer beträgt die Empfindlichkeit etwa 2,5 V/Oktave. Ohne Stecker liegt intern +5 V an: Vom Mittelrastpunkt aus kann der Regler die Sequenz bis zu eine Oktave auf- oder abwärts transponieren.', 'Pitch Add In is sampled at the start of each step; changes during the step are not followed. At full attenuator setting, sensitivity is approximately 2.5 V/octave. With no plug inserted, the input is normalled to +5 V: from the center detent, the control can transpose the sequence by up to one octave in either direction.'],
  ...
  },
  {
    ...
    paragraphs: [
      ['Reihe 2 erzeugt eine analoge Spannung und MIDI-Velocity. Im Normalmodus sind die Endwerte 1 und 127, die Mittelrastung liegt wegen mechanischer Toleranzen etwa bei 62–64. Add In wird zur Reglereinstellung addiert; 0–5 V entsprechen bei vollem Abschwächer dem beschriebenen Velocity-Bereich.', 'Row 2 produces an analog voltage and MIDI velocity. In Normal mode its endpoints are 1 and 127; mechanical tolerances put the center detent around 62–64. Add In is added to the knob setting; at full attenuator setting, 0–5 V corresponds to the described velocity range.'],
      ...
      ['In Dual und X-fade ist der zweite Kanal immer monophon: Er erhält nur die zuerst eingegebene Akkordnote. Damit werden höchstens 7 + 1 Noten pro Schritt ausgegeben. In den Split-Modi gehört die Mittelrastung zum Basis-Kanal. B4/B5 lassen Modulation auf den beiden Hälften gegensinnig wirken; B6/B7 gleichsinnig.', 'In Dual and X-fade, the second channel is always monophonic: it receives only the first-entered chord note. This limits output to 7 + 1 notes per step. In Split modes the center detent belongs to the base channel. B4/B5 make modulation act in opposite directions on the two halves; B6/B7 act in the same direction.'],
  ...
  },
  {
    ...
    paragraphs: [
      ...
      ['A und B sind gemeinsame Sammelschienen. Schritte 3–16 besitzen zusätzlich individuelle B-Buchsen, die ohne Stecker vom gemeinsamen B-Anschluss gespeist werden. Ohne externe Beschaltung liegt an A der Takt mit +12 V, an B kontinuierlich +5 V.', 'A and B are common buses. Steps 3–16 also have individual B jacks, normalled to the common B connection when unplugged. Without external patches, A carries the clock at +12 V and B carries continuous +5 V.'],
  ...
  },
  {
    ...
    paragraphs: [
      ['Globale Parameter werden bei jeder Änderung nichtflüchtig gespeichert. Die vier rechten globalen Schalter haben für Encoder-Einstellungen Priorität von rechts nach links: Ein Schalter außerhalb der Mitte sperrt die links davon liegenden Einstellfunktionen.', 'Global parameters are stored in nonvolatile memory whenever changed. For encoder settings, the four right-hand global switches have priority from right to left: a switch outside center blocks the setting functions to its left.'],
  ...
  },
  {
    ...
    rows: [
      ...
      [['12 Uhr / Mittelrastung', '12 o’clock / center detent'], ['1/8', '1/8']],
  ...
  },
  {
    ...
    paragraphs: [
      ['Die Tabelle übernimmt die Anschlussbereiche des Anhangs. Die intern an I/O4 A anliegende +12-V-Taktspannung ist separat in §2.4 beschrieben; sie ist keine Erweiterung des angegebenen Bereichs für externe I/O-Signale.', 'The table follows the appendix’s connection ranges. The internally normalled +12 V clock on I/O4 A is separately described in §2.4; it does not extend the stated range for external I/O signals.'],
